...
// ── Sources panel ──────────────────────────────────────────────────────────────
function SourcesPanel({ sources, onClose, T }) {
  return (
...
export default function Chat() {
  ...
  useEffect(() => {
    getMeetings()
      .then(m => {
        setMeetings(m)
        if (!selId && m.length > 0) setSelId(m[0].id)
      })
      .catch(() => setMeetings([]))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 80px)' }}>
      ...
      <PageHeader
        title="Meeting Chat"
        subtitle="Ask questions about your meetings using AI."
      />
      ...
      <div style={{
                      ...
                      : meetings.length === 0
                        ...
                        : meetings.map(m => (
                            <option key={m.id} value={m.id}>
                              {(m.filename || 'Untitled').slice(0, 26)}
                            </option>
                          ))
                    }
                  </select>
        ...
        <div style={{
          ...
          {/* Sources panel — slides in when sources selected */}
          {sources && (
            <SourcesPanel
              sources={sources}
